...
def print_status_table(status: dict) -> None:
    ...
    table.add_row("Profiles", ", ".join(profiles) if profiles else "(none)")
    auto_cap = status.get("auto_capture", True)
    table.add_row("Auto-capture", "enabled" if auto_cap else "disabled")
